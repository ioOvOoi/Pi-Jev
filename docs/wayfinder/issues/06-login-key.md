...
## Resolution（已复核：真人 /login + auth.json 落 key）
...
**真人复核 ✓（2026-09-17）**：TUI 内 `/login` 选 TypeSafe 输入真 key 后，`/jev` 面板渲染 `key: ✓ auth.json（apikey…3ba9）`——key 已落 `~/.pi/agent/auth.json`，插件按 provider 读回成功。（auth.json 属敏感路径，取证以面板输出为准，不直读文件。）

—— 代码侧结题。

（复检）凭据库与 env 皆无 key 时面板渲染 `key: ✗ 未配置 —— 运行 pi login 选 TypeSafe，或设 TYPESAFE_API_KEY`，missing 态符合预期（env 态另有 ts-env 探针证据）。三源（auth.json / env / missing）复核完毕，本票结题。
