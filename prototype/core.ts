...
export async function loadConfig(): Promise<JevConfig> {
  ...
  if (process.env.PI_JEV_MAX_CONCURRENT)
    cfg.maxConcurrent = +process.env.PI_JEV_MAX_CONCURRENT;
  // 配置文件后套，仅覆盖显式出现的字段
  try {
    const file = JSON.parse(
      await readFile(join(homedir(), ".pi/agent/pi-jev.json"), "utf8"),
    );
    ...
    if (file.lowConfidence)
      Object.assign(cfg.lowConfidence, file.lowConfidence);
  } catch {
    /* 文件缺失 = 全默认，正常路径 */
  }
  return cfg;
...
export async function resolveKey(): Promise<{
  ...
  try {
    // TODO(08): 换 ctx.modelRegistry.getApiKeyForProvider("typesafe")
    const auth = JSON.parse(
      await readFile(join(homedir(), ".pi/agent/auth.json"), "utf8"),
    );
    const k = auth?.typesafe?.key;
    if (k) return { key: k, source: "auth.json" };
  } catch {
    /* 无文件 */
  }
...
const mapHttp = (status: number): JevError["error"] =>
  status === 401
    ? {
        code: "auth",
        message: "TypeSafe key 缺失或无效",
        hint: "运行 pi login 选 TypeSafe，或设 TYPESAFE_API_KEY",
      }
    : status === 422
      ? { code: "validation", message: "请求体校验失败（见服务端返回）" }
      : status === 429
        ? { code: "rate_limited", message: "超限流" }
        : status === 529
          ? { code: "overloaded", message: "TypeSafe 过载" }
  ...
  return async (args: { state: unknown; questions: Record<string, any> }) => {
    if (!args?.state || !args?.questions || !Object.keys(args.questions).length)
      return {
        error: {
          code: "validation",
          message: "state 与 questions（至少一问）为必填",
        },
      } satisfies JevError;
    ...
    if (!key)
      return {
        error: {
          code: "auth",
          message: "未配置 TypeSafe key",
          hint: "pi login 选 TypeSafe，或设 TYPESAFE_API_KEY",
        },
      } satisfies JevError;
    ...
    try {
      // ponytail: 全局信号量而非按 provider 隔离，v0.1 单插件足够
...
const toSdkQuestion = (type: string, q: any) =>
  type === "noul"
    ? {
        ...
        criteria: { true: q.trueMeans ?? "", false: q.falseMeans ?? "" },
      }
    : type === "choice"
      ? { type, instructions: q.question, criteria: q.options }
...
export async function createJev(): Promise<JevClient> {
  return { cfg: await loadConfig(), client: new StubTypeSafeClient(DEFAULTS) };
}
...
if (process.argv[1]?.endsWith("core.ts")) {
  ...
  const r = await makeJevTool(
    ...
    questions: {
      dept: {
        question: "哪个团队处理？",
        options: { billing: "账务", technical: "故障", sales: null },
      },
    },
  });
