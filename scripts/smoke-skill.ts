...
const iso = { dir: join(tmp, "skills", "typesafe-ai"), statePath: join(tmp, "state.json") };

console.log("1) 隔离目录首次同步:", JSON.stringify(await syncSkill(iso)));
console.log("2) 隔离目录再同步(应 up-to-date):", JSON.stringify(await syncSkill(iso)));
...
console.log("   state:", SKILL_STATE_PATH, JSON.stringify(await readSkillState()));
